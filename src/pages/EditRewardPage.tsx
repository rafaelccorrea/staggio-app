...
export const EditRewardPage: React.FC = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { updateReward } = useRewardManagement();
  ...
  useEffect(() => {
    const loadReward = async () => {
      ...
      try {
        const data = await rewardsApi.getById(id);
        setReward(data);
        setFormData({
          name: data.name,
        ...
        toast.error('Erro ao carregar prêmio');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data: UpdateRewardRequest = {
      name: formData.name,
    ...
    setSubmitting(true);
    try {
      const result = await updateReward(id, data);
      if (result) {
        navigate('/rewards/manage');
      }
    } finally {
  ...
  if (loading) {
    return (
      <Layout>
        <LoadingContainer>Carregando prêmio...</LoadingContainer>
      </Layout>
    );
  }
    ...
    <Layout>
      <PageContainer>
        <PageContent>
          ...
          <PageHeader>
            <PageTitleContainer>
              <PageTitle>Editar Prêmio</PageTitle>
              <PageSubtitle>Altere as informações do prêmio</PageSubtitle>
            </PageTitleContainer>
          ...
          <ContentWrapper>
                ...
                <RowContainer>
                  ...
                  <FieldContainer>
                    ...
                    <FieldInput
                      ...
                      disabled={submitting}
                    />
                    <Hint>Deixe vazio para usar ícone padrão</Hint>
                  </FieldContainer>
                </RowContainer>
              </Section>
                ...
                <RowContainer>
                  <FieldContainer>
                    ...
                    <NumericFormat
                      ...
                      disabled={submitting}
                    />
                    <Hint>Deixe vazio para ilimitado</Hint>
                  </FieldContainer>
                  ...
                  <FieldContainer>
                    ...
                    <NumericFormat
                      ...
                      disabled={submitting}
                    />
                    <Hint>Menor número aparece primeiro</Hint>
                  </FieldContainer>
                </RowContainer>
                ...
                <FieldContainer>
                  <CheckboxContainer>
                    <Checkbox
                      ...
                      disabled={submitting}
                    />
                    <CheckboxLabel>Prêmio ativo no catálogo</CheckboxLabel>
                  </CheckboxContainer>
                  <Hint>Desmarque para ocultar do catálogo</Hint>
                </FieldContainer>
              </Section>
            ...
            {/* Preview - Reutilizar componentes da CreateRewardPage */}
            <PreviewSection>
              <PreviewCard>
                <PreviewHeader>
                ...
                <PreviewContent>
                  ...
                  <PreviewInfo>
                    ...
                    {reward && (
                      <InfoItem>
                        <InfoLabel>Resgatados:</InfoLabel>
                        <InfoValue>{reward.redeemedCount}</InfoValue>
                      </InfoItem>
                    )}
                  </PreviewInfo>
                </PreviewContent>
...
// Styled components específicos do preview (compartilhados com CreateRewardPage)
...
const InactiveWarning = styled.div`
  padding: 0.625rem;
  background: #fee2e2;
